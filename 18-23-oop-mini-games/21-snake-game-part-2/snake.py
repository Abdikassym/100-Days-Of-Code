...
class Snake:
    ...
    def add_segment(self):
        self.extend(self.all_segments[-1].position())

    def is_collide_with_wall(self):
        if 290 >= self.head.xcor() >= -300 and 300 >= self.head.ycor() >= -280:
            return False
        else:
            return True
